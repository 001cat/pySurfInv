...
class BsplBasis(object):
    ...
    def __init__(self, z, n, deg=None, alpha=2.0, eps=np.finfo(float).eps) -> None:
        ...
        x[deg - 1] = 0.0
        x[deg:n] = (
            np.power(alpha, range(n - deg))
            * (alpha - 1)
            / (np.power(alpha, n - deg + 1) - 1)
        )
        x[n] = 1.0
    ...
    def plot(self):
        ...
        plt.figure()
        plt.plot(np.linspace(0, 1, self.n), self.basis.T)
...
""" Basic layers for Rayleigh wave inversion """
...
class SeisLayerVs(SeisLayer):
    ...
    def _bspl(self, z, nBasis, deg=None):
        ...
        if (
            hasattr(self, "_bspl_")
            and (nBasis == self._bspl_.nBasis)
            and (deg == self._bspl_.deg)
            and (len(z) == self._bspl_.n)
        ):
            pass
        else:
...
class Crust(SeisLayerVs):
    ...
    def _calVs(self, z, **kwargs):
        ...
        if self.parm.get("Gauss", False) is False:
            ...
            vs1 = gaussFun(
                self.parm["Gauss"][0], self.parm["Gauss"][1], self.parm["Gauss"][2], z
            )
            return vs0 + vs1
...
class OceanMantle(SeisLayerVs):
    ...
    def _calOthers(self, z, vs, **kwargs):
        ...
        qs = np.array([150.0] * len(z))
        qp = np.array([1400.0] * len(z))
        return vp, rho, qs, qp
...
class OceanSedimentCascadia(OceanSediment):
    ...
    def _calVs(self, z, **kwargs):
        vs = (
            0.02 * self._calH(**kwargs) ** 2 + 1.27 * self._calH(**kwargs) + 0.29 * 0.1
        ) / (self._calH(**kwargs) + 0.29)
...
class OceanMantleHybrid(OceanMantle):
    ...
    def _calVs(self, z, **kwargs):
        ...
        def getCrustH():
            h = np.diff(layersAbove[0])
            grp = np.array(layersAbove[6][:-1])  # z,vs,vp,rho,qs,qp,grp,layerName
            if (
                np.diff(np.insert(grp == "crust", [0, len(grp)], False)).sum() != 2
            ):  # check if there are two seperated layers of crust
        ...
        if conversionModel == "Yamauchi":
            seisMod = OceanSeisRuan(
                HSCM(age=max(1e-3, self.parm["ThermAge"]), zdeps=crustH + z, Tp=Tp),
                period=1,
            )
        elif conversionModel == "Ritzwoller":
            seisMod = OceanSeisRitz(
                HSCM(age=max(1e-3, self.parm["ThermAge"]), zdeps=crustH + z, Tp=Tp)
            )
        else:
        ...
        vs = merge2(
            z,
            seisMod.vs,
            self._bspl(z, nBasis) * np.array([0] + list(self.parm["Vs"])) + seisMod.vs,
            xL=zMelt,
            xH=(zMelt + crustH) * 1.7 - crustH,
        )
    ...
    def _calOthers(self, z, vs, **kwargs):
        ...
        Qage = (
            modelInfo.get("lithoAge", None)
            if modelInfo.get("lithoAgeQ", False)
            else None
        )
...
def buildSeisLayer(
    parm: dict, layerClass: type[SeisLayer], BrownianConvert=True
) -> SeisLayer:
...
if __name__ == "__main__":
    ...
    b = buildSeisLayer(
        {
            "H": 10,
            "Vs": [BrownianVar(3.2, 3.1, 3.3, 0.02), BrownianVar(3.7, 3.5, 3.9, 0.02)],
        },
        OceanCrust,
    )
    c = b.copy()
    ...
    a = buildSeisLayer(
        {"H": 200, "Vs": [-0.3, -0.2, -0.1], "ThermAge": 4}, OceanMantleHybrid
    )
    ...
    b = buildSeisLayer(
        {"H": 200, "Vs": [4.4, 4.1, 4.2, 4.4, 4.5], "ThermAge": 4}, OceanMantle
    )
