...
class CascadiaOcean(MCinv):
    def _loadLocalInfo(self, ymlDict, localInfo, layerClassDict):
        ...
        return localDict
    def isgood_new(self,verbose=False):
        if not super().isgood():
            return False
...
#         '''
#         Negative velocity gradient below moho, works if
#         'NegSlopeBelowCrust'=True in self.info['Info']
#         '''
#         if self.info.get('NegSlopeBelowCrust',False) is False:
#             pass
#         elif not vsMantle[1]<vsMantle[0]:
...
#         '''
#         Limit Vs variation within mantle, works if self.info['mantleVariationLimit'] is set, made by Ayu
#         '''
#         mantleVsVaryLimit = self.info.get('mantleVariationLimit',False)
#         if mantleVsVaryLimit:
#             if (vsMantle.max() - vsMantle.min()) > 0.15*vsMantle.mean():
#                 return False
...
#         # new constraints within mantle based on grids instead of layers
#         z,vs,_,_,_,_,grp = self.seisPropGrids()
#         iMantle = np.array(grp) == 'mantle'
...
#         No local maximum
#         '''
#         if isinstance(self.layers[-1], layerClassDefault['OceanMantle_ThermBsplineHybrid']):
#             if len(scipy.signal.argrelmax(vs)[0]) > 0:
#                 if verbose:
#                     print('Debug: shallow local maximum found')
#                 return False
